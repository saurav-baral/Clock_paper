# -*- coding: utf-8 -*-
"""
Created on Thu Jun  6 11:53:08 2024
...
output_location = f"/data/projects/saubar/Working_folder_circadian_rhythm/15.Timeout_Exon_Analysis/{family_group}"
genome_location = "/data/projects/saubar/Genomes_2023-12-26"
...
#     print(initial_list_of_species)
    ...
    list_of_species = random.sample(list_of_species, 2)
        ...
        #     query_length = query_length/3
            for offset in range(3):
                ...
                for i in range(len(translated_sequence)):
                    ...
                    if len(translated_sequence[i])> 0.5*query_length and  str(translated_sequence[i]).count("X") < 5:
...
def get_genomic_coordinates(output_location,error_exon,species,last_exon_number):
    ...
    print("ERROR EXON NUMBER= ",error_exon_number)
#     return
...
#             print(current_position, next_position)
            ...
            if error_exon == last_exon_number:
                ...
                if complement == "0":
                    ...
                    fragment_end = int(final_coordinate_file_lines[next_position].split(",")[2])+5000
                if complement == "1":
                    fragment_start = int(final_coordinate_file_lines[next_position].split(",")[3])-5000
                ...
                original_query_name =f"{'_'.join(final_coordinate_file_lines[current_position].split(',')[6].split('_')[:-1])}_{error_exon_number}"
        ...
        for records in genome_file:
            ...
            while True:
                ...
                if (records.seq[start-3:start-1]).lower() == "ag" and ag != "Y":
                    print((records.seq[start-3:start-1]), records.id, records.seq)
                    ag = "Y"
...
# error_file_lines = ["Boloria_euphrosyne,OX637870.1,000,000,0,Y,Error_Exon_13,00,00,00"]   
for line in error_file_lines:
    if len(line) != 1:
        ...
        with open(f"{output_location}/1.Blast_result/{species}/final_coordinates.csv", 'r') as final_coordinate_file:
            ...
            last_exon_number = f"Error_Exon_{len(final_coordinate_file_lines)-1}"
            print(last_exon_number)
        
        if line.split(",")[6] != last_exon_number:
            continue
