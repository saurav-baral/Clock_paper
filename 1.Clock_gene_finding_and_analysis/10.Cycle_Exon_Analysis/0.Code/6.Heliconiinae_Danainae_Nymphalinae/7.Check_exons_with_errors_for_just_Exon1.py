# -*- coding: utf-8 -*-
"""
Created on Thu Jun  6 10:11:13 2024
...
output_location = f"/mnt/griffin/saubar/Working_folder_circadian_rhythm/10.Cycle_Exon_Analysis/{family_group}"
...
#         print(f"{exon}.fa")
    ...
    list_of_species = random.sample(list_of_species, 1)
#     list_of_species = ["Colias_croceus"]
...
def get_genomic_coordinates(output_location,error_exon,species,last_exon_number):
...
#             print(current_position, next_position)
            ...
            if error_exon_number == "1":
                while final_coordinate_file_lines[current_position].split(",")[5] == "Y":
                ...
                if complement == "0":
                    fragment_start = int(final_coordinate_file_lines[current_position].split(",")[2])-(50000*(current_position-1))
                    fragment_end = final_coordinate_file_lines[current_position].split(",")[2]
                if complement == "1":
                    fragment_start = final_coordinate_file_lines[current_position].split(",")[3]
                    fragment_end = int(final_coordinate_file_lines[current_position].split(",")[3]) + (50000*(current_position-1))

                scaffold = final_coordinate_file_lines[current_position].split(",")[1]
                original_query_name =f"{'_'.join(final_coordinate_file_lines[current_position-1].split(',')[6].split('_')[:-1])}_{error_exon_number}"
                
                
            elif error_exon_number == last_exon_number:
                ...
                if complement == "0":
                    ...
                    fragment_end = int(final_coordinate_file_lines[next_position].split(",")[2])+2000
                if complement == "1":
                    fragment_start = int(final_coordinate_file_lines[next_position].split(",")[3])-2000
        ...
        #     query_length = query_length/3
            for offset in range(3):
                ...
                for i in range(len(translated_sequence)):
                    ...
                    if len(translated_sequence[i])> 0.8*query_length and  str(translated_sequence[i]).count("X") < 5 and "M" in translated_sequence[i]:
    ...
    #Simple check for lenghth
        ...
        for records in genome_file:
            ...
            while True:
                ...
                if ((records.seq[stop:stop + 2]).lower() == "gt" or (records.seq[stop:stop + 2]).lower() == "gc") and gt != "Y":
                    gt = "Y"
                elif gt != "Y":
                    stop +=3
        ...
        blast_command = f'blastn -query "{query_location}" -db "{genome}" -num_alignments 3 -out "{out_location}/blast_out.htm" -html'
        # print(blast_command)
        # subprocess.run(f'{blast_command}', shell = True, stderr = subprocess.DEVNULL)
        subprocess.run(f'{blast_command}', shell = True, stderr = subprocess.DEVNULL)
    
        blast_command = f'blastn -query "{query_location}" -db "{genome}" -num_alignments 3 -out "{out_location}/blast_out.txt"'
        subprocess.run(f'{blast_command}', shell = True, stderr = subprocess.DEVNULL)
...
for line in error_file_lines:
    if len(line) != 1 and line.split(",")[6] == "Error_Exon_1":
        print(line)
        ...
        left_oh,right_oh = make_query(output_location,error_exon,species)
        fragment_start, fragment_end, scaffold, complement,query_name = get_genomic_coordinates(output_location,error_exon,species,last_exon_number)
...
#         alignment_file = "alignment_clustal_Exon_1_translated_genomic_sequence_1_frame2.fa"
#         alignment_name = "alignment_clustal_Exon_1_translated_genomic_sequence_1_frame2"
