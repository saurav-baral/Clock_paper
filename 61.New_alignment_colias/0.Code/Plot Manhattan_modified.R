...
gg.manhattan <- function(df, threshold, hlight, col, ylims, title){
  ...
  df.tmp <- df %>% 
    ...
    mutate(is_sup_high = ifelse(!is.na(Rho) & Rho != "", "yes", "no"))
  ...
  labels_df <- df.tmp %>%
    ...
    mutate(label_fill = ifelse(is_sup_high == "yes", "red", "white"))
  ...
  ggplot(df.tmp, aes(x=BPcum, y=(P))) +
    ...
    #     filter(is_annotate == "yes") %>%
    #     distinct(SNP, .keep_all = TRUE),
    #   aes(label = as.factor(SNP), color = is_sup_high),
    #   size = 1.5,
    #   force = 20,
    #   # nudge_y = -0.05,
    #   max.overlaps = Inf,
    ...
    #   data = labels_df,
    #   aes(label = SNP),
    #   fill = labels_df$label_fill,  # now lengths match
    ...
    # geom_label_repel(
    #   data = df.tmp %>% distinct(SNP, .keep_all = TRUE),
    #   aes(label = SNP, color = is_sup_high),   # ← color depends on is_sup_high
    #   size = 1.5,
    #   force = 10,
    #   max.overlaps = Inf,
    #   alpha = 0.7,
...
dataset = read.table("C:/Users/sauba/Desktop/20.RERConverge_trimmed/3.New_colias/61.New_alignment_colias/7.Manhatten/dataset_5_papilio_unmerged.csv", header = T, sep = ",")
...
pdf("C:/Users/sauba/Desktop/20.RERConverge_trimmed/3.New_colias/61.New_alignment_colias/7.Manhatten/dataset_5_papilio_unmerged.pdf", width = 11, height = 2.5)
